...
class EdgeController(finger.FingerController):
    ...
    def GetForces(self, poses, vels, contact_point, slip_speed, pen_depth, N_hat):
        ...
        if contact_point is None:
            ...
            F_CN = (-F_GN*w_L**2 + 4*I_L*a_LNd - 2*dd_d_Nd*mu_SL*m_M*p_CN*w_L + 2*dd_d_Nd*mu_SL*m_M*p_LN*w_L + 2*dd_d_Nd*m_M*p_CT*w_L - 2*dd_d_Nd*m_M*p_LT*w_L + dd_d_Nd*m_M*w_L**2 - d_theta_L**2*mu_SL*h_L*m_M*p_CN*w_L + d_theta_L**2*mu_SL*h_L*m_M*p_LN*w_L - 2*d_theta_L**2*mu_SL*m_M*p_CN*r*w_L + 2*d_theta_L**2*mu_SL*m_M*p_CN*w_L*d_N + 2*d_theta_L**2*mu_SL*m_M*p_LN*r*w_L - 2*d_theta_L**2*mu_SL*m_M*p_LN*w_L*d_N + d_theta_L**2*h_L*m_M*p_CT*w_L - d_theta_L**2*h_L*m_M*p_LT*w_L + d_theta_L**2*h_L*m_M*w_L**2/2 + 2*d_theta_L**2*m_M*p_CT*r*w_L - 2*d_theta_L**2*m_M*p_CT*w_L*d_N - 2*d_theta_L**2*m_M*p_LT*r*w_L + 2*d_theta_L **
                    2*m_M*p_LT*w_L*d_N + d_theta_L**2*m_M*r*w_L**2 - d_theta_L**2*m_M*w_L**2*d_N - 4*d_theta_L*d_d_T*mu_SL*m_M*p_CN*w_L + 4*d_theta_L*d_d_T*mu_SL*m_M*p_LN*w_L + 4*d_theta_L*d_d_T*m_M*p_CT*w_L - 4*d_theta_L*d_d_T*m_M*p_LT*w_L + 2*d_theta_L*d_d_T*m_M*w_L**2 + 2*d_theta_L*b_P*w_L - 4*mu_SL*a_LNd*m_M*p_CN*w_L - 4*mu_SL*a_LNd*m_M*p_CN*d_T + 4*mu_SL*a_LNd*m_M*p_LN*w_L + 4*mu_SL*a_LNd*m_M*p_LN*d_T + a_LNd*m_L*w_L**2 + 4*a_LNd*m_M*p_CT*w_L + 4*a_LNd*m_M*p_CT*d_T - 4*a_LNd*m_M*p_LT*w_L - 4*a_LNd*m_M*p_LT*d_T + 2*a_LNd*m_M*w_L**2 + 2*a_LNd*m_M*w_L*d_T)/(w_L*(-2*mu_SL*p_CN + 2*mu_SL*p_LN + 2*p_CT - 2*p_LT + w_L))

            F_CT = (F_GN*mu_SM*w_L**2 - 4*I_L*mu_SM*a_LNd - 2*dd_d_Td*mu_SL*m_M*p_CN*w_L + 2*dd_d_Td*mu_SL*m_M*p_LN*w_L + 2*dd_d_Td*m_M*p_CT*w_L - 2*dd_d_Td*m_M*p_LT*w_L + dd_d_Td*m_M*w_L**2 + 2*d_theta_L**2*mu_SL*m_M*p_CN*w_L**2 + 2*d_theta_L**2*mu_SL*m_M*p_CN*w_L*d_T - 2*d_theta_L**2*mu_SL*m_M*p_LN*w_L**2 - 2*d_theta_L**2*mu_SL*m_M*p_LN*w_L*d_T - 2*d_theta_L**2*m_M*p_CT*w_L**2 - 2*d_theta_L**2*m_M*p_CT*w_L*d_T + 2*d_theta_L**2*m_M*p_LT*w_L**2 + 2*d_theta_L**2*m_M*p_LT*w_L*d_T - d_theta_L**2*m_M*w_L**3 - d_theta_L**2*m_M*w_L**2*d_T + 4*d_theta_L*d_d_N*mu_SL*m_M*p_CN*w_L - 4*d_theta_L*d_d_N*mu_SL *
                    m_M*p_LN*w_L - 4*d_theta_L*d_d_N*m_M*p_CT*w_L + 4*d_theta_L*d_d_N*m_M*p_LT*w_L - 2*d_theta_L*d_d_N*m_M*w_L**2 - 2*d_theta_L*mu_SM*b_P*w_L - 2*mu_SL*a_LNd*h_L*m_M*p_CN + 2*mu_SL*a_LNd*h_L*m_M*p_LN - 4*mu_SL*a_LNd*m_M*p_CN*r + 4*mu_SL*a_LNd*m_M*p_CN*d_N + 4*mu_SL*a_LNd*m_M*p_LN*r - 4*mu_SL*a_LNd*m_M*p_LN*d_N - mu_SM*a_LNd*m_L*w_L**2 + 2*a_LNd*h_L*m_M*p_CT - 2*a_LNd*h_L*m_M*p_LT + a_LNd*h_L*m_M*w_L + 4*a_LNd*m_M*p_CT*r - 4*a_LNd*m_M*p_CT*d_N - 4*a_LNd*m_M*p_LT*r + 4*a_LNd*m_M*p_LT*d_N + 2*a_LNd*m_M*r*w_L - 2*a_LNd*m_M*w_L*d_N)/(w_L*(-2*mu_SL*p_CN + 2*mu_SL*p_LN + 2*p_CT - 2*p_LT + w_L))

            tau_M = (-F_GN*mu_SM*p_CN*w_L**2 + F_GN*mu_SM*p_MN*w_L**2 + 4*I_L*mu_SM*a_LNd*p_CN - 4*I_L*mu_SM*a_LNd*p_MN - 2*I_M*dd_theta_Md*mu_SL*p_CN*w_L + 2*I_M*dd_theta_Md*mu_SL*p_LN*w_L + 2*I_M*dd_theta_Md*p_CT*w_L - 2*I_M*dd_theta_Md *
                     p_LT*w_L + I_M*dd_theta_Md*w_L**2 + 2*d_theta_L*mu_SM*b_P*p_CN*w_L - 2*d_theta_L*mu_SM*b_P*p_MN*w_L + mu_SM*a_LNd*m_L*p_CN*w_L**2 - mu_SM*a_LNd*m_L*p_MN*w_L**2)/(w_L*(-2*mu_SL*p_CN + 2*mu_SL*p_LN + 2*p_CT - 2*p_LT + w_L))
            tau_M = tau_M[0]
            self.last_d_T = d_T
            self.last_d_N = d_N
